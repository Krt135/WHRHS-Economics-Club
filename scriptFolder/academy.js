...
// Initialize using the imported object
...
let lessons     = {};   // keyed by Firebase push-ID
let currentId   = null; // Firebase key of open lesson
let topicFilter = "all";
let levelFilter = "all";
let sortMode    = "newest";
let quizState   = {};
let qbQuestions = [];   // quiz builder rows in create modal

const CURRENT_USER = { name: "Kartikeya Pant", initials: "KP" }; // swap for real auth later

// ─────────────────────────────────────────────
//  FIREBASE LISTENERS
//  onValue fires once immediately and then on
//  every change, so the UI always stays in sync.
// ─────────────────────────────────────────────
...
onValue(ref(db, "lessons"), snapshot => {
  ...
  if (currentId) {
    // If we're viewing a lesson, refresh it live
    if (lessons[currentId]) renderLesson();
    else showList();           // deleted by someone else
  } else {
    renderList();
  }
});
...
function renderList() {
  ...
  // Convert object → array for filtering/sorting
  ...
  else items.sort((a,b) => b.postedAt - a.postedAt);  // newest

  const el = document.getElementById("lessonsList");
  if (!items.length) {
    el.innerHTML = `<div class="empty-state">
      <div class="empty-italic">Educational lessons coming soon.</div>
      <div class="empty-sub">Exec members will publish simplified economics lessons here.</div>
    </div>`;
    return;
  }
...
function renderLesson() {
  ...
  // Parse === headings === and [EXAMPLE] boxes
  ...
  if (qs.length) {
    ...
    if (done) {
      ...
      quizHtml = `<div class="quiz-section">
        <div class="quiz-header">✅ COMPREHENSION CHECK</div>
  ...
  // Comments — stored as object in Firebase, convert to array
  ...
  document.getElementById("lessonBody").innerHTML = `
    ...
    <div class="key-concepts">
      ...
      <ul class="kc-list">${concepts.map(c=>`<li>${esc(c)}</li>`).join("")}</ul>
    </div>` : ""}
    <div class="lesson-divider"></div>
    <div class="lesson-content">${parseContent(l.content)}</div>
    ${quizHtml}
    <div class="lesson-reaction">
      <button class="react-btn ${l.liked?"liked":""}" onclick="reactLesson('like')">
        👍 Helpful (${l.likes||0})
      </button>
      <button class="react-btn ${l.disliked?"disliked":""}" onclick="reactLesson('dislike')">
...
async function publishLesson() {
  ...
  const concepts = document.getElementById("cConcepts").value
  ...
  const lessonData = {
    ...
    content,
    quiz:     validQuiz,
    postedAt: Date.now(),       // Unix ms — easy to sort
    likes:    0,
    dislikes: 0,
    liked:    false,
    disliked: false
    // comments will be added as a sub-collection via push()
  };

  // push() auto-generates a unique key under /lessons
  ...
  // renderList() fires automatically via onValue
}

// ─────────────────────────────────────────────
//  EDIT LESSON  →  Firebase update
// ─────────────────────────────────────────────
...
async function saveEdit() {
  const l = lessons[currentId]; if (!l) return;
  const concepts = document.getElementById("eConcepts").value
    .split("\n").map(s=>s.trim()).filter(Boolean);
  ...
  await update(ref(db, `lessons/${currentId}`), {
    ...
    concepts,
    content:  document.getElementById("eContent").value.trim() || l.content,
  });
  closeModal("editModal");
  // renderLesson() fires automatically via onValue
}

// ─────────────────────────────────────────────
//  DELETE LESSON  →  Firebase remove
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
//  REACTIONS  →  Firebase update
// ─────────────────────────────────────────────

async function reactLesson(type) {
  const l = lessons[currentId]; if (!l) return;
  let { likes=0, dislikes=0, liked=false, disliked=false } = l;
  ...
  await update(ref(db, `lessons/${currentId}`), { likes, dislikes, liked, disliked });
}

// ─────────────────────────────────────────────
//  COMMENTS  →  Firebase push / remove / update
// ─────────────────────────────────────────────
...
async function postComment() {
  ...
  // Push to /lessons/{id}/comments
  const cmtRef = push(ref(db, `lessons/${currentId}/comments`));
  await set(cmtRef, commentData);
  inp.value = "";
  // renderLesson() fires automatically via onValue
}
...
async function deleteComment(lessonKey, commentKey) {
  await remove(ref(db, `lessons/${lessonKey}/comments/${commentKey}`));
}

// ─────────────────────────────────────────────
//  QUIZ (client-side only — not stored in DB)
// ─────────────────────────────────────────────
...
function renderQuizBuilder() {
  ...
  el.innerHTML = qbQuestions.map((qq, qi) => `
    <div class="qb-question">
      ...
      <input class="form-input" style="margin-bottom:8px;font-size:14px"
        placeholder="Question text..." value="${esc(qq.q)}"
        oninput="qbQuestions[${qi}].q=this.value">
      <div class="qb-opts">
        ${qq.opts.map((o,oi) => `
          <div class="qb-opt-row">
            <input class="qb-opt-input" placeholder="Option ${oi+1}" value="${esc(o)}"
              oninput="qbQuestions[${qi}].opts[${oi}]=this.value">
            <input type="radio" class="qb-correct-radio" name="correct_${qi}"
              ${qq.correct===oi?"checked":""}
              onchange="qbQuestions[${qi}].correct=${oi}" title="Mark as correct">
            <span class="qb-correct-label">✓ correct</span>
          </div>`).join("")}
      </div>
      <input class="form-input" style="margin-top:8px;font-size:13px"
        placeholder="Explanation (shown after answering)..." value="${esc(qq.exp)}"
        oninput="qbQuestions[${qi}].exp=this.value">
    </div>`).join("");
}
...
Object.assign(window, {
  ...
  answerQuiz, nextQuiz,
  addQuizQuestion, renderQuizBuilder,
  updateWC, updateEWC,
  postAnnounce,
  sortMode,        // read by select onchange
});

// Expose sortMode setter so the <select> onchange can update it
window.setSortMode = function(val) { sortMode = val; renderList(); };